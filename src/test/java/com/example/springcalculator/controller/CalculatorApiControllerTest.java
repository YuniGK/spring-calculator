...
SpringBootTest는 실제와 마찬가지로 어플리케이션을 시작하며 테스트
AutoconfigureWebMvc는 서버를 시작하지 않고 테스트
*/
...
public class CalculatorApiControllerTest {
    ...
    @BeforeEach
    public void init(){
        Mockito.when(marketApi.connect()).thenReturn(1350);
    }
    ...
    public void sumTest() throws Exception {
        ...
        */
        mockMvc.perform(
                ...
                MockMvcResultMatchers.status().isOk()//상태 코드 200
        ).andExpect(
                MockMvcResultMatchers.content().string("27000")
        ).andDo(
